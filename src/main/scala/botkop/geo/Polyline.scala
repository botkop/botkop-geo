...
object Polyline {
  ...
  def apply(polyline: String, precision: Double): Polyline = {
    new Polyline(decode(polyline, precision), polyline)
  }

  def apply(path: List[LatLng], precision: Double): Polyline = {
    new Polyline(path, encode(path, precision))
  }

  def apply(polyline: String): Polyline = { // Scala does not allow overloading with default args
    new Polyline(decode(polyline), polyline)
  }

  def apply(path: List[LatLng]): Polyline = { // Scala does not allow overloading with default args
    new Polyline(path, encode(path))
  }
  ...
  def encode(coordinates: List[LatLng], precision: Double = 0.0): String = {
    ...
    val factor = Math.pow(10, if (precision == 0.0) 5 else precision)
    var output: String = encode(coordinates.head.lat, factor) + encode(
      coordinates.head.lng,
      factor)
